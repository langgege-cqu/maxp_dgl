...
def train(model_cfg, dataset_cfg, optimizer_cfg, criterion_cfg, device, graph_data):
    ...
    output_folder = os.path.join(dataset_cfg['OUT_PATH']+'_'+str(dataset_cfg['K_FOLD']), 'models')
    ...
    for epoch in range(dataset_cfg['EPOCHS']):
        ...
        model_path = os.path.join(output_folder,
                                  'neighbor_label_gatv2_deepwalk2_{}_epoch{:02d}_val{:04d}.pth'.format(dataset_cfg['MODEL_PREFIX'], epoch + 1,
                                                                        int(val_acc * 10000)))
