...
from unicmp_1 import UniCMP2
from unicmp import UniCMP
from util import load_dgl_graph, load_dgl_graph_2
...
def init_model_2(model_cfg, k_fold, k, device):
    if model_cfg['GNN_MODEL'] == 'unicmp':
        model = UniCMP2(input_size=model_cfg['INPUT_SIZE'], num_class=model_cfg['NUM_CLASS'],
    ...
    state = th.load(model_cfg['CHECKPOINTS'+str(k)][k_fold], map_location='cpu')
...
def init_model(model_cfg, k_fold, k, device):
    if model_cfg['GNN_MODEL'] == 'unicmp':
...
def test(model_cfg, dataset_cfg, device, graph_data):
    ...
    # 4fold gatv2 deepwalk2
    test_dataloader, node_feats, labels = get_dataloader(dataset_cfg, graph_data)
    k_fold_result = []
    for k_fold in range(len(model_cfg['CHECKPOINTS1'])):
        model = init_model_2(model_cfg, k_fold, 1, device)
        result = test_epoch(model, test_dataloader, node_feats, labels, model_cfg['NUM_CLASS'], device)
        k_fold_result.append(result)
    
    # 4fold gatv1 deepwalk2
    for k_fold in range(len(model_cfg['CHECKPOINTS2'])):
        model = init_model(model_cfg, k_fold, 2, device)
        result = test_epoch(model, test_dataloader, node_feats, labels, model_cfg['NUM_CLASS'], device)
        k_fold_result.append(result)
    
    # 4fold gatv1 deepwalk1
    # deepwalk features is change , reload dgl features
    graph_data = load_dgl_graph_2(dataset_cfg['DATA_PATH'], dataset_cfg['K_FOLD'])
    graph, labels, train_nid, val_nid, test_nid, node_feats = graph_data
    test_dataloader, node_feats, labels = get_dataloader(dataset_cfg, graph_data)
    
    for k_fold in range(len(model_cfg['CHECKPOINTS3'])):
        model = init_model(model_cfg, k_fold, 3, device)
        result = test_epoch(model, test_dataloader, node_feats, labels, model_cfg['NUM_CLASS'], device)
        k_fold_result.append(result)

    # 4fold gatv2 deepwalk1
    for k_fold in range(len(model_cfg['CHECKPOINTS4'])):
        model = init_model_2(model_cfg, k_fold, 4, device)
        result = test_epoch(model, test_dataloader, node_feats, labels, model_cfg['NUM_CLASS'], device)
        k_fold_result.append(result)
