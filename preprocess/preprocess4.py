...
label_path = os.path.join(base_path, publish_path, 'label.pkl')
label_idx = tr_val_labels_df[tr_val_labels_df.label == label].node_idx.to_numpy()
with open(label_path, 'wb') as f:
    pickle.dump({'tr_val_label_idx': label_idx, 
                 'test_label_idx': test_labels_idx,
                 'label': labels}, f)
    

print('=' * 50)
root_dir = os.path.join(base_path, publish_path, 'order_labels')
os.makedirs(root_dir, exist_ok=True)

for i in range(split):
    label_path = os.path.join(root_dir, 'label_split{}.pkl'.format(i + 1))
