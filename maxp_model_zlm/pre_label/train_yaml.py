...
def init_model(model_cfg, in_feat, device):
    if model_cfg['GNN_MODEL'] == 'graphsage':
        model = GraphModel(1,
                           ...
                           activation=F.relu,
                           input_drop=model_cfg['INPUT_DROP'],
        ...
        model = GNNModel(input_size=model_cfg['IN_FEAT'],
                         num_class=model_cfg['N_CLASS'],
        ...
        model = UnimpPlus(input_size=model_cfg['IN_FEAT'],
                          num_class=model_cfg['N_CLASS'],
                          num_layers=model_cfg['N_LAYERS'],
                          num_heads=model_cfg['NUM_HEADS'],
                          hidden_size=model_cfg['HIDDEN_DIM'])
    elif model_cfg['GNN_MODEL'] == 'unicmp':
        model = UniCMP(input_size=model_cfg['IN_FEAT'],
                       num_class=model_cfg['N_CLASS'],
                       num_layers=model_cfg['N_LAYERS'],
                       num_heads=model_cfg['NUM_HEADS'],
                       hidden_size=model_cfg['HIDDEN_DIM'],
        ...
        raise NotImplementedError(
            'So far, only support three algorithms: GraphSage, GraphConv, and GraphAttn')
...
def prep_optimizer(dataset_cfg, model, num_train_optimization_steps):
    ...
    optimizer_grouped_parameters = [{
        ...
        'lr': dataset_cfg['LEARNING_RATE'] * dataset_cfg['COEF_LR']
    }, {
    ...
    optimizer = OptimAdam(optimizer_grouped_parameters,
                          ...
                          warmup=dataset_cfg['WARMUP_PROPORTION'],
...
pre_labels_probs = np.load('../data/pre_label_probs.npy')
...
def load_subtensor(node_feats, labels, seeds, input_nodes, n_classes, device, training=False):
    ...
    input_pre_labels_probs = pre_labels_probs[input_nodes]
    input_pre_labels_probs = th.from_numpy(input_pre_labels_probs).float()

    dist = Categorical(input_pre_labels_probs)
    input_pre_labels = dist.sample()

    masklabels = labels.clone()
    masklabels[seeds] = -2  # mask 预测点label
    input_labels_mask = masklabels[input_nodes]

    input_labels = labels[input_nodes]

    count = torch.sum(input_labels == -1)
    print('=================', count, input_labels.shape, count / input_labels.shape[0])

    rd_m = th.rand(input_labels.shape[0])
    un_label_mask = (input_labels == -1)
    mask = un_label_mask * (rd_m < 0.8)  # -1的0.8部分进行打标签
    input_labels[mask] = input_pre_labels[mask]
    # if training:
    #     rd_m = th.rand(input_labels.shape[0])
    #     un_label_mask = (input_labels == -1)
    #     mask = un_label_mask * (rd_m < 0.9)  # -1的0.9部分进行打标签
    #     input_labels[mask] = input_pre_labels[mask]
    # else:
    #     un_label_mask = (input_labels == -1)  # 验证时候全打
    #     input_labels[un_label_mask] = input_pre_labels[un_label_mask]

    input_labels[input_labels_mask < 0] = n_classes
        ...
        if (step + 1) % dataset_cfg['GRADIENT_ACCUMULATION_STEPS'] == 0:
            ...
            if global_step % log_step == 0:
                logging.info("Epoch: %d/%s, Step: %d/%d,  Loss: %f, Acc: %f, Time/step: %f",
                             ...
                             (log_step * dataset_cfg['GRADIENT_ACCUMULATION_STEPS']))
                start_time = time.time()
...
def train(model_cfg, dataset_cfg, device, graph_data):
    ...
    num_train_optimization_steps = (
        int(len(train_dataloader) + dataset_cfg['GRADIENT_ACCUMULATION_STEPS'] - 1) /
    ...
    output_folder = dataset_cfg['OUT_PATH'] + str(k_fold)
    os.makedirs(output_folder, exist_ok=True)

    global_step, best_records = 0, [-1, 0.0]
    for epoch in range(dataset_cfg['EPOCHS']):
        tr_loss, tr_acc, global_step = train_epoch(epoch, model, train_dataloader, node_feat,
                                                   labels, optimizer, criterion,
                                                   model_cfg['N_CLASS'], device, global_step,
                                                   dataset_cfg['LOG_STEP'])
        logging.info("Train Epoch %d/%s Finished | Train Loss: %f | Train Acc: %f ", epoch + 1,
                     dataset_cfg['EPOCHS'], tr_loss, tr_acc)

        val_loss, val_acc = val_epoch(model, val_dataloader, node_feat, labels, criterion,
                                      model_cfg['N_CLASS'], device)
        ...
        model_path = os.path.join(
            output_folder, 'pre_label_2layer_gat_sage_dgl_model_epoch{:02d}'.format(epoch + 1) +
            '_valloss_{:.4f}'.format(val_loss) + '_val_{:.4f}'.format(val_acc) + '.pth')
