...
class GraphSageLayer(nn.Module):
    def __init__(self, in_feats, out_feats, activation=F.elu, aggregator_type='lstm', dropout=0):
        ...
        self.graph = dglnn.SAGEConv(in_feats=in_feats,
                                    out_feats=out_feats,
...
class GraphAttnLayer(nn.Module):
    def __init__(self, in_feats, out_feats, num_heads, activation=F.elu, attn_drop=0, dropout=0):
        ...
        self.graph = dglnn.GATConv(in_feats=in_feats,
    ...
    def __init__(self,
            ...
            for i in range(num_layers):
                if i == 0:
                    self.graphsage.append(
                        GraphSageLayer(input_size, hidden_size, dropout=graph_drop))
                else:
                    self.graphsage.append(
                        GraphSageLayer(hidden_size, hidden_size, dropout=graph_drop))
        if use_conv:
            self.graphconv = nn.ModuleList()
            for i in range(num_layers):
                if i == 0:
                    self.graphconv.append(
                        GraphConvLayer(input_size, hidden_size, dropout=graph_drop))
                else:
                    self.graphconv.append(
                        GraphConvLayer(hidden_size, hidden_size, dropout=graph_drop))
            ...
            for i in range(num_layers):
                if i == 0:
                    self.graphattn.append(
                        GraphAttnLayer(input_size,
                                       hidden_size,
                                       num_heads=num_heads,
                                       dropout=graph_drop))
                else:
                    self.graphattn.append(
                        GraphAttnLayer(hidden_size,
                                       hidden_size,
                                       num_heads=num_heads,
                                       dropout=graph_drop))
        ...
        if use_densenet:
            ...
            for i in range(num_layers):
                for j in range(i + 1):
                    if j == 0:
                        self.graphskip.append(
                            ShortCutLayer(input_size, hidden_size, dropout=graph_drop))
                    else:
                        self.graphskip.append(
                            ShortCutLayer(hidden_size, hidden_size, dropout=graph_drop))
        elif use_resnet:
            self.graphskip = nn.ModuleList()
            for i in range(num_layers):
                if i == 0:
                    self.graphskip.append(ShortCutLayer(input_size, hidden_size,
                                                        dropout=graph_drop))
                else:
                    self.graphskip.append(
                        ShortCutLayer(hidden_size, hidden_size, dropout=graph_drop))
