...
def init_model(model_cfg, in_feat, device):
    if model_cfg['GNN_MODEL'] == 'graphsage':
        ...
        model = GNNModel(input_size=model_cfg['IN_FEAT'], num_class=model_cfg['N_CLASS'], num_layers=model_cfg['N_LAYERS'],
                           num_heads=model_cfg['NUM_HEADS'], hidden_size=model_cfg['HIDDEN_DIM'],
                           label_drop=model_cfg['LABEL_DROP'], feat_drop=model_cfg['FEAT_DORP'], drop=model_cfg['DROP_RATE'])
    elif model_cfg['GNN_MODEL'] == 'unimpplus':
        model = UnimpPlus(input_size=model_cfg['IN_FEAT'], num_class=model_cfg['N_CLASS'], num_layers=model_cfg['N_LAYERS'],
                           num_heads=model_cfg['NUM_HEADS'], hidden_size=model_cfg['HIDDEN_DIM'])
    elif model_cfg['GNN_MODEL'] == 'unicmp':
        model = UniCMP(input_size=model_cfg['IN_FEAT'], num_class=model_cfg['N_CLASS'],
                       num_layers=model_cfg['N_LAYERS'], num_heads=model_cfg['NUM_HEADS'],
                       hidden_size=model_cfg['HIDDEN_DIM'], label_drop=model_cfg['LABEL_DROP'],
                       feat_drop=model_cfg['FEAT_DORP'], graph_drop=model_cfg['GRAPH_DORP'],
                       attn_drop=model_cfg['ATTN_DROP'], drop=model_cfg['DROP_RATE'], 
