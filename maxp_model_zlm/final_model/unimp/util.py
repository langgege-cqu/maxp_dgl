...
def load_dgl_graph(dataset_cfg):
    graph = dgl.load_graphs(dataset_cfg['GRAPH_PATH'])[0][0]
    graph = dgl.to_bidirected(graph, copy_ndata=True)
    print('################ Graph info: ###############')
    print(graph)

    with open('{}{}.pkl'.format(dataset_cfg['LABEL_PATH'], dataset_cfg['K_FOLD']), 'rb') as f:
    ...
    node_feat = th.from_numpy(np.load(dataset_cfg['NODE_PATH'])).float()
    walk_feat = th.from_numpy(np.load(dataset_cfg['WALK_PATH'])).float()
    edge_feat = th.from_numpy(np.load(dataset_cfg['EDGE_PATH'])).float()
