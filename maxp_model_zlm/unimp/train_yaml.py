...
from sampler import MultiLayerInOutSampler
...
def train_epoch(
    ...
    for step, (input_nodes, seeds, blocks) in enumerate(train_dataloader):
        input_feats, input_labels, batch_labels = load_subtensor(
            dataset_cfg, node_feats, labels, seeds, input_nodes, device, mask=True
        )
...
def val_epoch(model, val_dataloader, dataset_cfg, node_feats, labels, criterion, device):
    ...
    with th.no_grad():
        for step, (input_nodes, seeds, blocks) in enumerate(val_dataloader):
            input_feats, input_labels, batch_labels = load_subtensor(
                dataset_cfg, node_feats, labels, seeds, input_nodes, device, mask=False
            )
            blocks = [block.to(device) for block in blocks]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--k_fold', type=int, default=2, help="k_fold")
