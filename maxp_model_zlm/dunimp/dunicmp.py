            ...
            for i in range(num_layers):
                if i == 0:
                    self.graphattn.append(
                        GraphAttnLayer(input_size, hidden_size, num_heads=num_heads, dropout=graph_drop)
                    )
                else:
                    self.graphattn.append(
                        GraphAttnLayer(hidden_size, hidden_size, num_heads=num_heads, dropout=graph_drop)
                    )
...
class DouUniCMP(nn.Module):
    ...
    def __init__(
        ...
        super(DouUniCMP, self).__init__()

        self.InUniCMP = UniCMP(
    ...
    def forward(self, in_blocks, in_input_feats, in_input_labels, out_blocks, out_input_feats, out_input_labels):
        in_feats = self.InUniCMP(in_blocks, in_input_feats, in_input_labels)
        out_feats = self.OutUniCMP(out_blocks, out_input_feats, out_input_labels)
        out = self.head(torch.cat((in_feats, out_feats), dim=-1))
        return out