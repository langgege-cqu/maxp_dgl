...
class SimSiam(nn.Module):
    def __init__(self, in_feats, hidden_dim, n_layers, heads, activation, feat_drop, attn_drop,
        ...
        self.f = nn.Sequential(
            ...
            nn.BatchNorm1d(hidden_dim * heads // 2), nn.ReLU(inplace=True),
            nn.Linear(hidden_dim * heads // 2, hidden_dim * heads // 4, bias=False),
            nn.BatchNorm1d(hidden_dim * heads // 4, affine=False))

        self.h = nn.Sequential(
            nn.Linear(hidden_dim * heads // 4, hidden_dim * heads // 2, bias=False),
            nn.BatchNorm1d(hidden_dim * heads // 2), nn.ReLU(inplace=True),
            nn.Linear(hidden_dim * heads // 2, hidden_dim * heads // 4))

    def forward(self, blocks, features, input_labels):
        x = self.encoder(blocks, features, input_labels)
        z = self.f(x)
        p = self.h(z)
        return p, z
...
class SimSiamLoss(nn.Module):
    ...
    def forward(self, p_1, z_1, p_2, z_2):
        z_1 = z_1.detach()
        z_2 = z_2.detach()

        loss = -(self.criterion(p_1, z_2).mean() + self.criterion(p_2, z_1).mean()) * 0.5
        return loss
...
def train_epoch(epoch, train_cfg, model, train_dataloader, node_feat, labels, optimizer, criterion1,
    ...
    for step, batch_data in enumerate(train_dataloader):
        ...
        p_1, z_1 = model(blocks, input_feats, input_labels)
        p_2, z_2 = model(_blocks, _input_feats, _input_labels)

        loss = criterion1(p_1, z_1, p_2, z_2)
...
def train(model_cfg, dataset_cfg, train_cfg, device, graph_data, k_fold):
    ...
    model = SimSiam(in_feats=gat_cfg['IN_FEATS'],
