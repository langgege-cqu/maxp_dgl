...
class GraphAttnLayer(nn.Module):
    def __init__(self, in_feats, out_feats, num_heads, gatv2=True,
                 activation=F.elu, attn_drop=0, dropout=0):
...
class UniMP(nn.Module):
    def __init__(self, input_size, feat_size, num_class=23, num_layers=2, num_heads=8,
                 ...
                 use_resnet=True, use_densenet=True, gatv2=True
                 ):
        ...
        if use_attn:
            ...
            for i in range(num_layers):
                if i == 0:
                    self.graphattn.append(GraphAttnLayer(input_size, hidden_size, num_heads=attn_heads, 
                                                         gatv2=gatv2, dropout=graph_drop))
                else:
                    self.graphattn.append(GraphAttnLayer(hidden_size, hidden_size, num_heads=attn_heads, 
                                                         gatv2=gatv2, dropout=graph_drop))        
