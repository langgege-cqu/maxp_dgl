...
def init_model(model_cfg, device):
    if model_cfg['GNN_MODEL'] == 'unicmp':
        model = UniCMP(input_size=model_cfg['INPUT_SIZE'], feat_size=model_cfg['FEAT_SIZE'],
                       num_class=model_cfg['NUM_CLASS'], edge_type=model_cfg['EDGE_TYPE'],
                       num_layers=model_cfg['NUM_LAYERS'], num_heads=model_cfg['NUM_HEADS'],
                       hidden_size=model_cfg['HIDDEN_SIZE'], attn_heads=model_cfg['ATTN_HEADS'],
                       label_drop=model_cfg['LABEL_DROP'], feat_drop=model_cfg['FEAT_DORP'],
                       graph_drop=model_cfg['GRAPH_DORP'], attn_drop=model_cfg['ATTN_DROP'],
...
def get_dataloader(dataset_cfg, graph, nid, drop=False):
    sample_graph = copy.copy(graph)

    if drop:
        node_num = sample_graph.number_of_edges(etype='cite')

        node_index = th.arange(node_num)
        rd_m = th.rand(node_num)
        drop_node = node_index[rd_m < dataset_cfg['EDGE_DROP']]

        sample_graph.remove_edges(drop_node, 'cite')
        sample_graph.remove_edges(drop_node, 'cited')
        sample_graph.remove_edges(drop_node + node_num, 'undirected')
        sample_graph.remove_edges(drop_node, 'undirected')
    
    sample_graph = dgl.add_self_loop(sample_graph, etype='cite')
    sample_graph = dgl.add_self_loop(sample_graph, etype='cited')
    sample_graph = dgl.add_self_loop(sample_graph, etype='undirected')
